...
abstract class AbstractIOUringChannel extends AbstractChannel implements UnixChannel {
    ...
    private long pollInUserData;
    private long pollOutUserData;
    private long pollRdhupUserData;
    private long connectUserData;
    ...
    protected void doClose() throws Exception {
        ...
        try {
            ...
            if (registration != null) {
                if (cancelConnect && connectUserData != 0) {
                    // Best effort to cancel the
                    registration.submit(IOUringIoOps.newAsyncCancel(
                            fd().intValue(), 0, connectUserData, Native.IORING_OP_CONNECT));
                }
    ...
    abstract class AbstractUringUnsafe extends AbstractUnsafe implements IOUringIoHandle {
        ...
        public void handle(IoRegistration registration, IoEvent ioEvent) {
            ...
            if (event == IOUringIoEvent.IO_LOOP_DONE_EVENT) {
                return;
            }
            ...
            switch (op) {
                ...
                case Native.IORING_OP_ASYNC_CANCEL:
                    if (data == Native.IORING_OP_CONNECT) {
        ...
        private void pollAddComplete(int res, short data) {
        ...
        public void close(ChannelPromise promise) {
            if (delayedClose == null) {
                ...
                delayedClose.addListener(new ChannelPromiseNotifier(false, promise));
                return;
            }

            super.close(newPromise());
        }
    ...
    private void submitConnect(InetSocketAddress inetSocketAddress) {
        ...
        IOUringIoOps ops = IOUringIoOps.newConnect(
                fd().intValue(), 0, remoteAddressMemoryAddress, registration.ioHandler().nextOpsId());
        registration.submit(ops);
        connectUserData = ops.udata();
    }
    ...
    protected final void doDeregister() {
        ...
        if ((ioState & POLL_RDHUP_SCHEDULED) != 0) {
            registration.submit(IOUringIoOps.newPollRemove(
                    fd().intValue(), 0, pollRdhupUserData, (short) Native.POLLRDHUP));
        }
        if ((ioState & POLL_IN_SCHEDULED) != 0) {
            registration.submit(IOUringIoOps.newPollRemove(
                    fd().intValue(), 0, pollInUserData, (short) Native.POLLIN));
        }
        if ((ioState & POLL_OUT_SCHEDULED) != 0) {
            registration.submit(IOUringIoOps.newPollRemove(
                    fd().intValue(), 0, pollOutUserData, (short) Native.POLLOUT));
        }
