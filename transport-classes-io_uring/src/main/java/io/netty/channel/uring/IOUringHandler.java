...
public final class IOUringHandler implements IoHandler, CompletionCallback {
    ...
    private final ArrayDeque<DefaultIoUringIoRegistration> touchedRegistrations;
    ...
    IOUringHandler(RingBuffer ringBuffer) {
        ...
        touchedRegistrations = new ArrayDeque<>();
    ...
    public int run(IoExecutionContext context) {
        ...
        while (completionQueue.hasCompletions()) {
            completed += completionQueue.process(this);
        }
        notifyIoFinished();
        return completed;
    }
    ...
    private void notifyIoFinished() {
        ...
        while ((handle = touchedRegistrations.poll()) != null) {
            handle.ioLoopCompleted();
        }
    }
    ...
    public void handle(int res, int flags, byte op, int fd, short data) {
        ...
        touchedRegistrations.offer(registration);
    ...
    private final class DefaultIoUringIoRegistration extends AtomicBoolean implements IOUringIoRegistration {
        ...
        void ioLoopCompleted() {
            handle.handle(this, IOUringIoEvent.IO_LOOP_DONE_EVENT);
        }
    }
    ...
    short nextOpsId() {
        // Compute the next ops id, but skip 0, because that's the default value for lastConnectId.
        opsId++;
        if (opsId == 0) {
            opsId++;
        }
